...
export default async function ConnectOuraPage({
  searchParams,
}: {
  ...
  const authHref = connect
    ? `/api/integrations/oura/auth?connect=${encodeURIComponent(connect)}`
    : null
  ...
  return (
    <PaperPage
      ...
      action={authHref ? { href: authHref, label: 'connect with oura' } : null}
      missing="This link is missing its token. Ask Dinghy for a fresh one."
      fine="You sign in with Oura itself. Dinghy never sees your password and only reads your data."
    />
  )
}
