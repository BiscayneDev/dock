...
/**
 * Insert the delivery id. Returns true when this delivery is new and should
 * be processed, false when it is a redelivery of an already-seen message.
 * On a transient database error, returns true: a duplicate reply is a
 * better failure mode than a silently dropped message (the send side is
 * still outbox-guarded).
 */
export async function claimInboundDelivery(messageId: string, chatGuid: string): Promise<boolean> {
    const supabase = createServerClient()
    const { error } = await supabase
        .from('spectrum_inbound_dedupe')
        .insert({ message_id: messageId, chat_guid: chatGuid })
    if (!error) return true
    if (error.code === '23505') return false // unique violation: redelivery
    console.error('dedupe insert failed (processing anyway):', error.message)
    return true
}
