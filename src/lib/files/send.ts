...
export interface HostedSendable {
    ...
    subtitle?: string
    format: FileFormat
    hosted: { url: string; expiresAt: string }
}
...
/** History line for a hosted file: lets the model find the link later (revoke_file). */
export function hostedHistoryLine(file: HostedSendable): string {
    const until = short(file.hosted.expiresAt)
    return `[file: ${file.title}] ${file.hosted.url}${until ? ` (expires ${until})` : ''}`
}
